...
public class ListRiskASKTableModel extends KTableSortedModel {
	...
	public ASRiskTextCellRenderer m_NizakRenderer = new ASRiskTextCellRenderer(ASRiskTextCellRenderer.INDICATION_FOCUS, SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_BOTTOM, Display.getDefault().getSystemColor(SWT.COLOR_DARK_YELLOW) );
	public ASRiskTextCellRenderer m_SrednjiRenderer = new ASRiskTextCellRenderer(ASRiskTextCellRenderer.INDICATION_FOCUS, SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_BOTTOM, Display.getDefault().getSystemColor(SWT.COLOR_RED) );
	public ASRiskTextCellRenderer m_VisokRenderer = new ASRiskTextCellRenderer(ASRiskTextCellRenderer.INDICATION_FOCUS, SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_BOTTOM, Display.getDefault().getSystemColor(SWT.COLOR_DARK_RED) );
...
//		setContentAt(1, 12, "1150");
		...
		setContentAt(2, 3, "Zastoj poslovnih procesa zbog loše dokumentiran og softvera");
		setContentAt(2, 4, "Nedokumentiranost Rijeènika podataka (Data Dictionary)");
		setContentAt(2, 5, "Nepostojanje mehanizama ispravnog adresiranja i prijenosa poruka");
		setContentAt(2, 6, "Neuèinkovita kontrola pristupa za nove aplikacije u Banci");
		setContentAt(2, 7, "Lokacija u podrucju podložnom poplavi");
		setContentAt(2, 8, "Nekorištenje alata za nadzor integriteta podataka i ispitivanje ranjivosti.");
		setContentAt(2, 9, "Curenje informacija sa stare raèunalne opreme Banke");
...
//		setContentAt(5, 12, "2");
		...
		setContentAt(6, 5, "Nepostojanje mehanizama ispravnog adresiranja i prijenosa poruka");
		setContentAt(6, 6, "Akti (procedure, pravilnici) neprikladni, zastarjeli, teško razumljivi");
...
//    			rowsX2.put(x + "/" + row+"",table_.getModel().getContentAt(x2, row));
	...
	public KTableCellRenderer doGetCellRenderer(int col, int row) {
		...
		if (content.get(7+"/"+row)=="Nizak")
				{
			return m_NizakRenderer;
		}

		if (/*getContentAt(5, row) == "4"*/content.get(7+"/"+row)=="Srednji") {
...
//			        for (int j=getFixedHeaderColumnCount(); j<getColumnCount(); j++)
		...
		if (/*getContentAt(5, row) == "3"*/content.get(7+"/"+row)=="Visoki") {
