...
public class KTableActionHandler {
    ...
    public KTableSelectAllRowsAction m_SelectAllRowsAction;
    ...
    /**ONLY SELECT ALL ACTION **/
    public KTableActionHandler(KTable table, String doSomething) {
        m_table = table;
...
//        m_SelectAllAction.run();
    ...
    /**ONLY SELECT ALL ROWS ACTION **/
    public KTableActionHandler(KTable table, String doSomething, int clickedColumn) {
        m_table = table;
//        m_SelectAllAction = new KTableSelectAllAction();
//        m_SelectAllAction.run();
        m_SelectAllRowsAction = new KTableSelectAllRowsAction();
        m_SelectAllRowsAction.run();
    ...
    protected class KTableSelectAllRowsAction extends Action {
		protected KTableSelectAllRowsAction() {
			setId("KTableSelectAllRowsAction");//$NON-NLS-1$
			setEnabled(false);
//			setText("Select All");
		}
		public void run() {
		...
		protected void selectAllRows(KTableModel model) {
		    ...
		    for (int row=model.getFixedHeaderRowCount(); row<model.getRowCount(); row++)
		         {
		            currentCell = model.belongsToCell(m_clickedColumn, row);
		            if (currentCell.x==m_clickedColumn && currentCell.y==row)
		                sel.add(currentCell);
		        }
		    try {
		        m_table.setRedraw(false);
	...
	protected void registerActionUpdaterSelectAll() {
	    m_table.addCellSelectionListener(new KTableCellSelectionListener() {
