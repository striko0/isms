...
*/
package hr.ante.test.renderers;
 ...
 */
public class ASCurrencyTextCellRenderer extends DefaultCellRenderer {
    ...
    public ASCurrencyTextCellRenderer(int style, int alignment) {
        super(style);        
...
//        setAlignment(SWTX.ALIGN_HORIZONTAL_RIGHT | SWTX.ALIGN_VERTICAL_BOTTOM);
    ...
    public void drawCell(GC gc, Rectangle rect, int col, int row, Object content, 
		...
		if (content.toString().contains("-")) {
			textColor = Display.getDefault().getSystemColor(SWT.COLOR_RED);
        ...
        if (focus && (m_Style & INDICATION_FOCUS)!=0) {
            ...
            drawCellContent(gc, rect, content.toString(), null, textColor, COLOR_BGFOCUS);
//            drawCellContent(gc, rect, content.toString(), null, textColor, COLOR_BGFOCUS);
            ...
            // draw content:
            drawCellContent(gc, rect, content.toString(), null, /*COLOR_FGROWFOCUS*/textColor, COLOR_BGROWFOCUS);
